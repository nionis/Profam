...
/***/ function(module, exports, __webpack_require__) {
	...
	var profanity = function () {
	  function profanity() {
	    ...
	    this.modes = new Map([['grawlix', { 'enabled': 1 }], ['obscure', { 'enabled': 0 }], ['choice', { 'enabled': 0 }], ['grawlix', { 'enabled': 0 }], ['asterisks', { 'enabled': 0 }], ['empty', { 'enabled': 0 }], ['black', { 'enabled': 0 }]]);

	    this.modes = (_modes = { // 'obscure', 'choice', 'grawlix', 'asterisks', 'empty', 'black' : Can hold multiple modes
	      'grawlix': 1,
	      'obscure': 0,
	      'choice': 0
	    }, _defineProperty(_modes, 'grawlix', 0), _defineProperty(_modes, 'asterisks', 0), _defineProperty(_modes, 'empty', 0), _defineProperty(_modes, 'black', 0), _modes);
	    ...
	    value: function downloadLocales() {
	      ...
	      if (locales.length) {
	        locales.filter(function (locale) {
	          return !(_this.downloadedLocales.indexOf(locale) !== -1);
	        }).forEach(function (locale) {
	          var url = _this.makeUrl(locale);
	          var self = _this;

	          _axios2.default.get(url).then(function (response) {
	            self.downloadedLocales.push(locale);
	            self.locales[locale] = response.data;
	      ...
	      if (modes !== null) {
	        ...
	        if (modes.length) {
	          if (isAdd) {
	            ...
	            (_modes2 = modes).push.apply(_modes2, _toConsumableArray(this.modes));
	          }
